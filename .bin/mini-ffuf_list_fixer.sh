...
for item in "${list_array[@]}"; do
    ...
    # If item does not start or end with slash, continue
    ...
    # replace item with temp_item in the list
    list_array=("${list_array[@]/$item/$temp_item}")

    # print status, use -r 
    printf "\rStatus: %d/%d" "$count" "${#list_array[@]}"
...
printf '%s\n' "${list_array[@]}" | sort -u > "$list_dir/fixed_ffuf_$(basename "$list_file")"
